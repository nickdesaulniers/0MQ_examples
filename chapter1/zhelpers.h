...
#include <uuid/uuid.h>
  ...
  error "Please upgrade to ZeroMQ/3.2 or higher"
#endif

// Provide a random number from 0..(num - 1)
...
static void s_dump (void* socket) {
  ...
  while (1) {
    ...
    int is_text = 1;
    int char_nbr = 0;
    for (; char_nbr < size; char_nbr++) {
      if ((unsigned char) data[char_nbr] < 32 ||
        (unsigned char) data[char_nbr] > 127) is_text = 0;
    }
    printf("[%03d]", size);
...
// Set simpel random printable identity on socket
...
// Print formatted string to stdout, prefixed by date.time and terminated with
// a new line.
